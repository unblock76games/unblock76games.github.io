...
gamepad.resize = function () {
  ...
  const h = gamepad.container.clientHeight * 0.15 * gamepad.scale;
  ...
  gamepad.ctx.fillStyle = "rgba(48, 47, 77, 1)";
  ...
  if (gamepad.numOfPlayers === 1) {
    ...
    const jumpSpace = h * 0.66;
    gamepad.buttons[0].setTransform(w - h * 3.8 - jumpSpace, h * 0.1, h * 1.2, h * 0.8); //P1 left
    gamepad.buttons[1].setTransform(w - h * 2.6 - jumpSpace, h * 0.1, h * 1.2, h * 0.8); //P1 right
    ...
    gamepad.buttons[5].setTransform(h * 2.6 + jumpSpace, h * 0.1, h * 1.2, h * 0.8); //P2 jump
  }
...
gamepad.hide = function () {
  gamepad.canvas.style.bottom = "-15%";
  gamepad.enabled = false;
};
