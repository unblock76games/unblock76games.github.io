...
function pixelsToMeters(iPixel){
    //carspritewidth : carrealmeter = pixel : meter
    //370:4,5 = 1 : x
    return (iPixel * PIXEL_TO_METER_RATIO);
};

function kMetersHToPixelsF(iKMeter){
    var iMeter = iKMeter*1000;
    var iPixel = (iMeter * METER_TO_PIXEL_RATIO); /// 1/4.5 = 0.222222
    var iPixelPerFrame = iPixel/(3600)/FPS;
    
    return iPixelPerFrame;
};

function framesToMilliSeconds (iFrames){
    return (iFrames * FPS_TIME).toFixed(0);
};

function milliSecondsToFrames (iMs){
    return (iMs * 0.001 * FPS).toFixed(0);
};
