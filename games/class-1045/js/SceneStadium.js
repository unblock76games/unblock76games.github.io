class SceneStadium extends Phaser.Scene {
    ...
    preload() {
      this.load.image('arrowsToMove','content/arrowsToMove.png');
    ...
    create() {
      ...
      if(!musicLoaded){
        ...
        this.container.y = 0;
        this.fieldBg = this.add.sprite(-113,0,'fieldBg');
        this.fieldBg.setOrigin(0,0);
        this.field = this.add.sprite(0,0,fieldKey);
        ...
        this.container.add([this.fieldBg,this.field,this.firstDownLine, this.lineOfScrimmage, this.circle, this.circle2, this.targetA, this.targetS, this.targetD]);
        //this.ball.anims.play('ballFlip');
        this.scoreBoard = this.add.container(213, 0);
        //adjust for the camera move to the right...
        this.scoreboardBg = this.add.image(320,0,'scoreboard');
        this.scoreboardBg.setOrigin(0.5,0);
        this.gmDropdown = this.add.image(-211,480, 'gmDropdown');
        this.gmDropdown.setOrigin(0,1);
        //this.gmDropdown.setScale(0.75);
        this.gmDropdown.visible = false;

        if(!mobile && newGame){
          this.arrowsToMove = this.add.sprite(520,320,'arrowsToMove');
        }
        ...
        if(mobile){
          ...
          this.dpadBase = this.add.image(205,400,'dpadBase');
          this.dpadTop = this.add.sprite(205,400,'dpadTop').setInteractive({ draggable: true });
          //this.dpadTop.setInteractive();
          this.dpadTop.on('drag', function (pointer, dragX, dragY) {
            this.x = pointer.x;
            this.y = pointer.y;
            if(pointer.x < 195){ mobileLeft = true;}else{mobileLeft = false;}
            if(pointer.x > 215){ mobileRight = true;}else{mobileRight = false;}
        ...
        //
        //this.helmetHome = this.add.image(-3,1,helmetHomeKey);
        this.helmetHome = this.add.image(-3,1,[helmetHomeKey + '2'])
        this.helmetHome.setOrigin(0,0);
        this.helmetHome.setScale(0.8,0.8);
        //this.helmetHome.setTexture(helmetHome);
        //this.helmetVisitor = this.add.image(825,1,helmetVisitorKey);
        this.helmetVisitor = this.add.image(825,1,[helmetVisitorKey + '2']);
        this.helmetVisitor.setOrigin(0,0);
        this.helmetVisitor.setScale(0.8,0.8);
        ...
        this.textScoreTeam1 = this.add.text(60, 20, scoreTeam1, { fontFamily: 'Arial', fontSize: 18, color: '#ffffff' });
        //this.textScoreTeam1.font = "Arial";
        this.textScoreTeam1.setOrigin(0.5, 0.0);
        this.textScoreTeam1.setShadow(-2, 2, '#333333', 2, true, true);
        this.textScoreTeam2 = this.add.text(580, 20, scoreTeam2, { fontFamily: 'Arial', fontSize: 18, color: '#ffffff' });
        this.textScoreTeam2.font = "Arial";
        this.textScoreTeam2.setOrigin(0.5, 0.0);
        this.textScoreTeam2.setShadow(-2, 2, '#333333', 2, true, true);
        this.textYardline = this.add.text(-100, 10, "Ball on the " + yardline, { fontFamily: 'Arial', fontSize: 12, color: '#ffffff' });
        //this.textYardline.font = "Arial";
        this.textYardline.setOrigin(0.0, 0.0);
        this.textYardline.setShadow(-2, 2, '#333333', 2, true, true);
        this.textYardsToGo = this.add.text(533, 5, "1st Down", { fontFamily: 'Arial', fontSize: 18, color: '#ffffff' });
        this.textYardsToGo.setOrigin(0.5, 0.0);
        this.textYardsToGo.setStroke('#000000', 3);
        this.textYardsToGo.setShadow(-2, 2, '#333333', 2, true, true);
        //stats...
        ...
        this.stats = this.add.text(106, 280, "GAME STATS", { fontFamily: 'Arial', fontSize: 12, color: '#ffffff' });
        this.stats.setOrigin(0, 0.0);
        this.stats.setStroke('#000000', 1);
        this.stats.setShadow(-2, 2, '#333333', 2, true, true);
        //this.stats.setTintFill(0xffffff, 0xff0000, 0x0000ff, 0x0000ff);
        this.statsInterceptions = this.add.text(110, 310, "Interceptions", { fontFamily: 'Arial', fontSize: 12, color: '#ffffff' });
        this.statsInterceptions.setOrigin(0.0, 0.0);
        this.statsInterceptionsNum = this.add.text(200, 310, statsInterceptions, { fontFamily: 'Arial', fontSize: 12, color: '#ffffff' });
        this.statsInterceptionsNum.setOrigin(1.0, 0.0);
        this.statsPassYards = this.add.text(110, 335, "Pass Yards", { fontFamily: 'Arial', fontSize: 12, color: '#ffffff' });
        this.statsPassYards.setOrigin(0.0, 0.0);
        this.statsPassYardsNum = this.add.text(200, 335, statsPassYards, { fontFamily: 'Arial', fontSize: 12, color: '#ffffff' });
        this.statsPassYardsNum.setOrigin(1.0, 0.0);
        this.statsRushYards = this.add.text(110, 360, "Rush Yards", { fontFamily: 'Arial', fontSize: 12, color: '#ffffff' });
        this.statsRushYards.setOrigin(0.0, 0.0);
        this.statsRushYardsNum = this.add.text(200, 360, statsRushYards, { fontFamily: 'Arial', fontSize: 12, color: '#ffffff' });
        this.statsRushYardsNum.setOrigin(1.0, 0.0);
        this.statsCompletions = this.add.text(110, 385, "Completions", { fontFamily: 'Arial', fontSize: 12, color: '#ffffff' });
        this.statsCompletions.setOrigin(0.0, 0.0);
        this.statsCompletionsNum = this.add.text(200, 385, statsCompletions, { fontFamily: 'Arial', fontSize: 12, color: '#ffffff' });
        this.statsCompletionsNum.setOrigin(1.0, 0.0);
        this.statsTouchdowns = this.add.text(110, 410, "Touchdowns", { fontFamily: 'Arial', fontSize: 12, color: '#ffffff' });
        this.statsTouchdowns.setOrigin(0.0, 0.0);
        this.statsTouchdownsNum = this.add.text(200, 410, statsTouchdowns, { fontFamily: 'Arial', fontSize: 12, color: '#ffffff' });
        this.statsTouchdownsNum.setOrigin(1.0, 0.0);
        this.statsTotalYards = this.add.text(110, 435, "Total Yards", { fontFamily: 'Arial', fontSize: 12, color: '#ffffff' });
        this.statsTotalYards.setOrigin(0.0, 0.0);
        this.statsTotalYardsNum = this.add.text(200, 435, statsTotalYards, { fontFamily: 'Arial', fontSize: 12, color: '#ffffff' });
        this.statsTotalYardsNum.setOrigin(1.0, 0.0);
        if(time == 300){this.textTime = this.add.text(533, 25, '5:00', { fontFamily: 'Arial', fontSize: 15, color: '#ffff00' });}
        if(time == 600){this.textTime = this.add.text(533, 25, '10:00', { fontFamily: 'Arial', fontSize: 15, color: '#ffff00' });}
        if(time == 900){this.textTime = this.add.text(533, 25, '15:00', { fontFamily: 'Arial', fontSize: 15, color: '#ffff00' });}
        this.textTime.setOrigin(0.5, 0.0);
        this.textTime.setStroke('#000000', 1);
        this.textTime.setShadow(-1, 1, '#333333', 1, true, true);

        this.btnSound = this.add.sprite(690,5,"buttonSound1");
        ...
        this.scoreBoard.add([this.scoreboardBg,this.textYardline,this.gmDropdown,this.btnSound,this.helmetHome,this.textScoreTeam1,this.textScoreTeam2]);

        this.speedBarMask = this.add.sprite(108,40,'speedBarMask');
        this.speedBarMask.setOrigin(0.0,0.0);
        //this.speedBarMask.setAngle(90);
        this.speedBar = this.add.sprite(110,42,'speedBar');
  ...
  if(player2 == ballCarrier){
    ...
    update(){
      ...
      if(time <= 0 && !ballSnapped){
        ...
        //this actually shouldn't be here I think, so it should probably stay commented out since 2024...
        //gameStop();
        //end poki
        //
        this.qb.destroy();
