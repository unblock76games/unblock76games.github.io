function CRoad(oRoadDrawingLevel, oElementContainer, iLevel, oSubwaterLevel){    
    var _iTrackLength;      // z length of entire track (computed)
    var _iCameraDepth;
    var _iWaterFrameToRender;
    var _iWaterFrameToRenderLod1;
    var _iFPSCounter;
    
    var _aSegments;         // array of road segments
    
    var _oTideController;
    
    this._init = function(oRoadDrawingLevel, oElementContainer, iLevel){       
        _iTrackLength = null;
        _iCameraDepth = CAMERA_DEPTH;
        _iWaterFrameToRender = 0;
        _iWaterFrameToRenderLod1 = 0;
        _iFPSCounter = 0;

        _oTideController = new CTideController(); 
    ...
    this.findRelativeSegment = function(z) {
        
        var oSegment;
        if(z<0){
            oSegment = _aSegments[Math.floor( _aSegments.length + (z/SEGMENT_LENGTH)% _aSegments.length)];
        }else {
            oSegment = _aSegments[Math.floor(z/SEGMENT_LENGTH) % _aSegments.length];
        }
    ...
    this.addSegment = function(curve, y) {
      ...
      _aSegments.push({
         index: n,
            p1: { world: {x: this.lastX(), y: this.lastY(),  z:  n   *SEGMENT_LENGTH }, camera: {}, screen: {}, backup:{} },
            p2: { world: {x: this.lastX() + curve , y: y,        z: (n+1)*SEGMENT_LENGTH }, camera: {}, screen: {}, backup:{} },
         curve: curve,
         sprites: [],
         cars: [],
         coins: [],
    ...
    this.clearVisual = function(oPlayerPos){
        ...
        _oTideController.flattenTide();
    };
   
    this.waterAnimationMovement = function(){
        _iFPSCounter++;
        if(_iFPSCounter>= 3){
            _iFPSCounter = 0;
            _iWaterFrameToRender++;
            if(_iWaterFrameToRender >= 10){
                _iWaterFrameToRender = 0;
            }
            if(_iWaterFrameToRender%3 === 0){
                _iWaterFrameToRenderLod1++;
                if(_iWaterFrameToRenderLod1 >= 10){
                    _iWaterFrameToRenderLod1 = 0;
                }       
            }
        }
    ...
    this.setTide = function(oStartPos, iLength, iHeight){
        
        ////RISING PHASE
        for(var n=0; n<iLength; n++){
            var oSegment = this.findRelativeSegment(oStartPos + n*SEGMENT_LENGTH);
            oSegment.p1.world.y = Util.easeOut(0, iHeight, n/iLength*2);
            oSegment.p2.world.y = Util.easeOut(0, iHeight, (n+1)/iLength*2);
        }
        ////FALL PHASE
        for(var n=iLength; n<iLength*2; n++){
            var oSegment = this.findRelativeSegment(oStartPos + n*SEGMENT_LENGTH);
            oSegment.p1.world.y = 0;
            oSegment.p2.world.y = 0;
        }
    };
    ...
    this.update = function(oPlayerPos){
        ...
        this.waterAnimationMovement();
        
        var playerY = oPlayerPos.y;
        ...
        for(n = 0 ; n < DRAW_DISTANCE ; n++) {
            ...
            Render.segment(oRoadDrawingLevel, CANVAS_WIDTH, NUM_LANES,
                           ...
                           segment.p2.screen.w,
                           segment.color,
                           n<10 ? s_oSpriteLibrary.getSprite('caustics_lod0_'+_iWaterFrameToRender) :  s_oSpriteLibrary.getSprite('caustics_lod0_'+_iWaterFrameToRenderLod1)
                                   )
        ...
        for(n = (DRAW_DISTANCE-1) ; n > 0 ; n--) {
            ...
            for(var i = 0 ; i < segment.cars.length ; i++) {
                ...
                /////// RENDER FOAM
                oElementContainer.setChildIndex(car.getFoam(), iDepth++);
                Render.sprite(car.getFoamDim(), car.getFoam(), spriteScale*car.getFoamProportion(), spriteX, spriteY, -0.5, -1, segment.clip);
            ...
            for(var i = 0 ; i < segment.sprites.length ; i++) {
                ...
                if(!sprite.grounded){
                    ...
                    spriteY     = Math.round((HALF_CANVAS_HEIGHT) - (segment.p1.screen.scale * (- yProject)  * HALF_CANVAS_HEIGHT));
                }
        ...
        //CLEAN SPRITES OUT OF SIGHT
        for(n = (DRAW_DISTANCE+20) ; n > DRAW_DISTANCE ; n--) {
            segment = _aSegments[(baseSegment.index + n) % _aSegments.length];
        ...
        if(s_oGame.getState() !== STATE_GAME_START){
            _oTideController.update();
        }
