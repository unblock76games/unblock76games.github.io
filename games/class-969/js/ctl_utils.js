...
function isExplorer(){
    return typeof document !== 'undefined' && !!document.documentMode && !isEdge();
};
...
function easeBackInQuart(t, b, c, d) {
	var ts=(t/=d)*t;
	var tc=ts*t;
...
var Util = {
    ...
    project: function(p, cameraX, cameraY, cameraZ, cameraDepth) {
      ...
      p.backup.y     = Math.round((HALF_CANVAS_HEIGHT) - (p.screen.scale * (- cameraY)  * HALF_CANVAS_HEIGHT));
    },
