function CLevelBuilder(oPlayer, aCars, oElementContainer, iLevel){
    ...
    this._addSprite = function(n, sprite, offset) {
        ...
        //////// Y OF THE ELEMENTS
        if(sprite.name.indexOf("buoy") !== -1){
            oElement.regY = -oSprite.height/4;
        }else {
            oElement.regY = -oSprite.height/3;
        }
        ...
        var bGrounded = false;
        if(sprite.name === SPRITES.PALM_TREE.name || sprite.name === SPRITES.TREE1.name || sprite.name === SPRITES.TREE2.name){
            bGrounded = true;
        }
        
        s_oGame.getSegments()[n].sprites.push({grounded:bGrounded, source: oElement, offset: offset, sprite:oSprite, collision:{center:iCollisionCenter, width:iCollisionWidth}});
    };
    ...
    */        
    ...
    this._initFinishLane = function(){        
        ...
        s_oGame.getSegments()[s_oGame.getSegments().length-1].sprites.push({ source: oElement, offset: 0, sprite:oSprite, collision:{center:0, width:0}});
    };
    ...
    this._initCars = function() {
        ...
        for (var n = 0 ; n < _iTotalCars ; n++) {
            ...
            sprite = Util.randomChoice(SPRITES.ENEMIES);
            //sprite = SPRITES.ENEMIES[0];
    ...
    this._initTerrainInfo = function(){
        ...
        TIDE_VARIABLE_FREQUENCY = LEVEL_INFO[iLevel].tide.frequency;
        TIDE_VARIABLE_HEIGHT = LEVEL_INFO[iLevel].tide.height;
