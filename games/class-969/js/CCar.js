...
function CCar(oSprite, iOffset, iZ, iSpeed, oElementContainer){
    ...
    this._init = function(oSprite, iOffset, iZ, iSpeed, oElementContainer){
        ...
        _iFoamProportion = _aSprite[CAR_CENTER].width/200; 

        _aFoamSprite = new Array();
        for(var i=0; i<29; i++){
            var oSprite = s_oSpriteLibrary.getSprite('foam_back_'+i);
            var oBitmap = createBitmap(oSprite);

            oBitmap.x = 500;
            oBitmap.regY = -10;
            _aFoamSprite.push(oBitmap);

            oElementContainer.addChild(oBitmap);
    ...
    this.getFoam = function(){
        return _aFoamSprite[_iCurFoamFrame];
    };
    
    this.getFoamDim = function(){
        return new createjs.Rectangle(0, 0, 128, 116);
    };
    
    this.getFoamProportion = function(){
        return _iFoamProportion;
    };
    ...
    this.setVisible = function(bVal){
        for(var i=0; i<_aFoamSprite.length; i++){
            _aFoamSprite[i].visible = false;
        }
        
        _oCar.visible = bVal;
        _aFoamSprite[_iCurFoamFrame].visible = bVal;
    };
    ...
    this.updateCarOffset = function(carSegment, oPlayer) {
        ...
        for(i = 2 ; i < lookahead ; i++) {
            aSegments = s_oGame.getSegments();
            segment = aSegments[(carSegment.index+i)%aSegments.length];

            if ((segment === oPlayer.getPlayerSegment()) && (_iSpeed > oPlayer.getCurSpeed()) && (Util.overlap(oPlayer.getPosition().x, oPlayer.getPlayerWidth(), _iOffset, carW, 0.8))) {
    ...
    this.update = function(dt, oPlayer){
        _iCurFoamFrame++;
        if(_iCurFoamFrame === _aFoamSprite.length){
            _iCurFoamFrame = 0;
        }
