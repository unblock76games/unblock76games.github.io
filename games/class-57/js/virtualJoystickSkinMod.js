...
(function (){
	...
	scope.placeSprites = function(){
		scope.canvasBackfill = scope.target.domElement.parentNode.appendChild(document.createElement('div'));
		scope.canvasBackfill.style.position = 'absolute';
		scope.canvasBackfill.style.background = 'black';
		scope.canvasBackfill.style.pointerEvents = 'none';
	...
	scope.resize = function(){
		...
		if(landscape){
			...
			scope.canvasBackfill.style.visibility = 'hidden';
			...
			const buttonOffset = Math.max(gap/2.9, 80);

			scope.skinnerObject.joystickElement.style.left = `${buttonOffset-edgeSize/2}px`;
			...
			scope.skinnerObject.buttonsElement.style.right = `${buttonOffset-edgeSize/2}px`;
			...
			scope.canvasBackfill.style.top = `${gap-edgeSize/2+scope.portraitOffset}px`;
			scope.canvasBackfill.style.visibility = 'visible';
			scope.canvasBackfill.style.width = `${frameHeight}px`;
			scope.canvasBackfill.style.height = `${frameHeight+edgeSize}px`;
